...
const REPLACE_DEV_DEPS = {
    ...
    'rollup-plugin-terser': {pkg: '@rollup/plugin-terser', version: 'latest'}
};

const PIN_DEPS = {
    '@aws-sdk/credential-providers': 'latest',
    '@rsbuild/plugin-styled-components': 'latest',
    'rollup-plugin-dts': 'latest'
};

// Patches for root-level npm overrides (simple string values only).
// `$rollup` = npm self-reference: resolves to whatever version root's
// devDependencies pin, avoiding EOVERRIDE when rollup is also a direct dep.
...
module.exports = {
    ...
    buildForceInstallArgs,
    REPLACE_DEV_DEPS,
    PIN_DEPS,
    OVERRIDE_PATCHES,
    FORCE_INSTALL_VERSIONS
};